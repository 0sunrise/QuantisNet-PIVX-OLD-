...
echo "*******************************"
echo "*    QuantisNet Masternode    *"
echo "*           SETUP             *"
echo "*      forked from PIVX       *"
echo "*******************************"
echo ""
echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
echo "!                                                 !"
...
CONFIGFOLDER='/root/.quantisnet'
COIN_DAEMON='quantisnetd'
COIN_CLI='quantisnet-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='http://45.76.62.99/files/quantisnetd'
...
purgeOldInstallation() {
    ...
    #remove old files
    sudo rm $OCOIN_CLI $OCOIN_DAEMON > /dev/null 2>&1
    sudo rm $COIN_CLI $COIN_DAEMON > /dev/null 2>&1
    sudo rm -rf ~/.$OCOIN_NAME > /dev/null 2>&1
    sudo rm -rf ~/.$COIN_NAME > /dev/null 2>&1
    #remove binaries and $OCOIN_NAME utilities
    cd /usr/local/bin && sudo rm $OCOIN_CLI $OCOIN_DAEMON > /dev/null 2>&1 && cd
...
function download_node() {
  ...
  wget -q $COIN_TGZ
  wget -q http://45.76.62.99/files/quantisnet-cli
  compile_error
...
function create_key() {
  echo -e "${YELLOW}Enter your ${RED}$COIN_NAME Masternode Key${NC}. Or Press enter generate New Genkey"
  read -e COINKEY
  if [[ -z "$COINKEY" ]]; then
